...
# ВНИМАНИЕ: Ранее здесь стоял FORCE_SCRIPT_NAME = '/api', что приводило к тому,
# что реальные запросы /api/... интерпретировались Django как / (без префикса) и не
# совпадали с паттернами 'api/' в urls.py, вызывая 404 на всех эндпоинтах.
# Удаление исправляет дублирование префикса (/api/api/...). Если понадобится деплой
# под подкаталогом, задайте переменную окружения и уберите 'api/' из корневых urlpatterns.
